...
const TopRated = ({ pageNo }) => {
  // Fetching top-rated TV shows using useQuery
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(
        `https://api.themoviedb.org/3/tv/top_rated?language=en-US&page=${pageNo}&api_key=${import.meta.env.VITE_API_KEY}`
      );
      if (!response.ok) {
