...
import MovieDetail from "../../MovieDetail/MovieDetail"; // Make sure this file exists
import TvShowsDetail from "../../TvShowDetail/TvShowDetail";

const TrendingTvShows = ({ pageNo }) => {
  // Fetching trending TV shows using useQuery
  ...
  const {
    ...
    queryKey: ["trendingTvShow", pageNo],
    queryFn: async () => {
      const response = await fetch(
        `https://api.themoviedb.org/3/tv/airing_today?language=en-US&page=${pageNo}&api_key=${import.meta.env.VITE_API_KEY}`
      );
      if (!response.ok) {
  ...
  return (
    <div className="flex flex-col items-center font-ptsans">
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 w-full px-4">
        {trendingTvShow?.results?.map((tvShow) => (
          <div
            key={tvShow.id}
