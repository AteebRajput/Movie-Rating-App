...
const PopularTvShows = ({ pageNo }) => {
  // Fetching popular TV shows using useQuery
  const [selectedTvShowId, setSelectedTvShow] = useState(null);
  console.log("In the Popular TV Shows");
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(
        `https://api.themoviedb.org/3/tv/popular?language=en-US&page=${pageNo}&api_key=${import.meta.env.VITE_API_KEY}`
      );
      if (!response.ok) {
